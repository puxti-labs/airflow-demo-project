...
```
Salesforce CRM          billing_sync             salesforce_sync
...
No column was renamed. No schema changed in BigQuery. But the *meaning* shifted — and every dbt model that uses `stg_opportunities.total_value` to measure revenue now relies on a Salesforce-side aggregation that can drift from the canonical source (`raw_order_lines`).
...
Declared once per source, typically at onboarding time:
